...
regressionFolder = ['day9-15merge', filesep, 'Reg Mats'];
...
regFieldNames = regexprep(regressionNames, '_[^_]*$', '');
...
for ii=1:length(daysToProcess)
    ...
    for jj=1:Nwbcfiles
        ...
        % tack on the regrssion results for each file
        for ll = 1:Nregressions
            regressionFullFile = [mainPath, filesep, regressionFolder, filesep, regressionNames{ll}, '.mat'];
            wbcData{jj}.Regs.(regFieldNames{ll}) = ...
                data_to_regressed_scored(wbcData{jj}, regressionFullFile);
        end
    end    
    ...
    for jj=1:Nmcf7files
        ...
        for ll = 1:Nregressions
            regressionFullFile = [mainPath, filesep, regressionFolder, filesep, regressionNames{ll}, '.mat'];
            mcf7Data{jj}.Regs.(regFieldNames{ll}) = ...
                data_to_regressed_scored(mcf7Data{jj}, regressionFullFile);
        end
    end
    ...
    for jj=1:Nmixfiles
        ...
        for ll = 1:Nregressions
            regressionFullFile = [mainPath, filesep, regressionFolder, filesep, regressionNames{ll}, '.mat'];
            mixData{jj}.Regs.(regFieldNames{ll}) = ...
                data_to_regressed_scored(mixData{jj}, regressionFullFile);
        end
    end
...
%% Similar to above code but plot the regressiosn instead
...
for ii = 1:Nregressions
        ...
        for kk = 1:length(daysToProcess)
            ...
            for ll = 1:length(WBCDataAll{kk})
                wbcDNDp = WBCDataAll{kk}{ll}.Channels.DAPI.totalSig_dBc > DNACutoff;
                Ndaywbc = Ndaywbc + sum(wbcDNDp);
                [n2, xout2] = hist(scale*WBCDataAll{kk}{ll}.Regs.(regFieldNames{ii})(wbcDNDp), numHistPts);                
                [n2, xout2] = makejagged(n2, xout2); 
                n2 = n2/sum(wbcDNDp);
                plot(xout2, n2, 'b'); hold on;    
            end
            ...
            for ll = 1:length(WBCDataAll{kk})
                wbcDNDp = WBCDataAll{kk}{ll}.Channels.DAPI.totalSig_dBc > DNACutoff;
                Nptsin = length(WBCDataAll{kk}{ll}.Regs.(regFieldNames{ii})(wbcDNDp));
                wbcFeatMerg(lastInd+[1:Nptsin]) = WBCDataAll{kk}{ll}.Regs.(regFieldNames{ii})(wbcDNDp); % the data
                wbcFeatLab(lastInd+[1:Nptsin]) = ll;
                lastInd = lastInd + Nptsin;
            end
            ...
            for ll = 1:length(MCF7DataAll{kk})
                mcf7DNDp = MCF7DataAll{kk}{ll}.Channels.DAPI.totalSig_dBc > DNACutoff;
                Ndaymcf7 = Ndaymcf7 + sum(mcf7DNDp);
                [n2, xout2] = hist(scale*MCF7DataAll{kk}{ll}.Regs.(regFieldNames{ii})(mcf7DNDp), numHistPts);
                [n2, xout2] = makejagged(n2, xout2); 
                n2 = n2/sum(mcf7DNDp);
                plot(xout2, n2, 'r'); hold on;    
            end
            ...
            for ll = 1:length(MCF7DataAll{kk})
                mcf7DNDp = MCF7DataAll{kk}{ll}.Channels.DAPI.totalSig_dBc > DNACutoff;
                Nptsin = length(MCF7DataAll{kk}{ll}.Regs.(regFieldNames{ii})(mcf7DNDp));
                mcf7FeatMerg(lastInd+[1:Nptsin]) = MCF7DataAll{kk}{ll}.Regs.(regFieldNames{ii})(mcf7DNDp); % the data
                mcf7FeatLab(lastInd+[1:Nptsin]) = ll;
                lastInd = lastInd + Nptsin;
            end
        ...
        a = annotation('textbox',[.15,.96,.4,.05],'String',[regFieldNames{ii}]);
        
        a.FontSize = 11;
        a.EdgeColor = 'none';
        
        saveas(gcf, [mainPath, filesep, 'VariationHist', filesep, regFieldNames{ii} '.png'], 'png');  
end
